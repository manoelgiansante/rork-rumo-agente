...
function getAvailablePorts() {
  for (let i = 0; i < MAX_CONTAINERS; i++) {
    ...
    const inUse = [...activeContainers.values()].some(c => c.noVncPort === noVncPort);
...
function getContainerStatus(userId) {
  ...
  const status = run(`docker inspect -f '{{.State.Status}}' ${name} 2>/dev/null`);
...
async function startDesktop(userId) {
  ...
  if (existing === 'running') {
    ...
    if (info) {
      ...
      return { status: 'running', noVncPort: info.noVncPort, vncPort: info.vncPort };
    }
    // Container running but not tracked - get ports from docker
    const ports = run(`docker port ${name} 6080 2>/dev/null`);
    if (ports) {
      const noVncPort = parseInt(ports.split(':').pop());
      const vncPorts = run(`docker port ${name} 5901 2>/dev/null`);
      const vncPort = vncPorts ? parseInt(vncPorts.split(':').pop()) : 5901;
      activeContainers.set(userId, {
        noVncPort, vncPort, lastActivity: Date.now(), containerId: name
      });
  ...
  if (activeContainers.size >= MAX_CONTAINERS) {
    ...
    const oldest = [...activeContainers.entries()]
  ...
  if (!ports) {
    throw new Error('Sem recursos disponíveis. Tente novamente em alguns minutos.');
  }
  ...
  const cmd = [
    'docker run -d',
    `--name ${name}`,
    `--hostname rumo-desktop`,
    `--memory=1g`,
    `--cpus=0.5`,
    `--shm-size=256m`,
    `-p ${ports.vncPort}:5901`,
  ...
  return { status: 'starting', noVncPort: ports.noVncPort, vncPort: ports.vncPort };
}
...
async function takeScreenshot(userId) {
  ...
  const result = run(
    `docker exec ${name} bash -c "export DISPLAY=:1 && scrot -o /tmp/screenshot.png 2>/dev/null && echo ok"`
  );
  ...
  run(`docker cp ${name}:/tmp/screenshot.png ${screenshotPath}`);
...
setInterval(() => {
  ...
  for (const [userId, info] of activeContainers.entries()) {
    if (now - info.lastActivity > INACTIVITY_TIMEOUT_MS) {
      console.log(`Auto-stopping inactive desktop for user ${userId.substring(0, 8)}...`);
      stopDesktop(userId).catch(err => console.error('Auto-stop error:', err));
    }
  }
}, 5 * 60 * 1000);
...
function syncContainers() {
  try {
    const output = run(`docker ps --filter "name=${CONTAINER_PREFIX}" --format "{{.Names}} {{.Ports}}" 2>/dev/null`);
    if (!output) return;
