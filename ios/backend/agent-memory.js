...
if (!process.env.CREDENTIAL_ENCRYPTION_KEY) {
  console.error(
    "[SECURITY CRITICAL] CREDENTIAL_ENCRYPTION_KEY não configurada! Credenciais NÃO serão criptografadas corretamente.",
  );
  console.error(
    "[SECURITY CRITICAL] Defina CREDENTIAL_ENCRYPTION_KEY no .env com exatamente 32 caracteres.",
  );
...
async function trackAppUsage(supabase, userId, appName) {
  await saveMemory(supabase, userId, "app_usage", appName, {
...
async function getWorkflowContext(supabase, userId) {
  const { data: workflows } = await supabase
    .from("learned_workflows")
    .select(
      "name, description, trigger_phrases, target_software, success_count",
    )
    .eq("user_id", userId)
    .eq("is_active", true)
    .order("success_count", { ascending: false })
  ...
  for (const action of actions) {
    ...
    if (action.tool === "run_command" && action.input.command) {
      ...
      learnings.push({
        ...
        key: `comando_${cmd}`,
        value: {
          full_command: action.input.command,
          context: userMessage.substring(0, 100),
        },
      });
    }
